...
`Redirect URLs` are the per-environment allow-list. If you include query parameters in `redirectTo`, the simplest safe setup is to allow the full local or deployed path pattern with `/**`. If you develop on `127.0.0.1`, a different port, or a Vercel preview URL, add those callback URL patterns too or Supabase will fall back to the `Site URL`.
